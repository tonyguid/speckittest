...
1. **ADO Feature Detection**: Check if the input matches ADO Feature ID pattern:
   - Pattern: `ADO:(\d+)` or `#(\d+)` at the start of input
   - If match found:
     a. Extract the numeric ID as ADO_FEATURE_ID
     b. Use the Azure DevOps MCP Server to fetch the feature with the matching id, and its parent epic
   - If no match: Use the input text as the feature description (ADO_ENABLED=false)
...
7. **Update local mirror file** ado-mirror.json with sync timestamp and lineage data
